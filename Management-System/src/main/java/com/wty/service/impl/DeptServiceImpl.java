...
public class DeptServiceImpl implements DeptService {
    ...
    //*该注解的两个属性 rollbackFor和propagation
    @Override
    @Transactional(rollbackFor = Exception.class)
    //默认出现RuntimeException才回滚  rollbackFor属性指定回滚的层级   这里全部异常都回滚
    public void deleteById(Integer id) {
        try {
            ...
            //insertLog   propagation属性设置为需要新事务  因为默认调用到是加入当前事务  如果失败了  记录日志的操作也会回滚
            //所以必须开新事务  propagation设为REQUIRES_NEW
            DeptLog deptLog = new DeptLog();
            deptLog.setCreateTime(LocalDateTime.now());
            deptLog.setDescription("尝试删除了" + id + "号部门的员工和部门数据");
            //调用另一个事务方法，考虑事务的传播行为
